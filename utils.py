...
class PrePrc():
    ...
    def stopw(self):
        ...
        stop_common_list = [ ind for ind in self.vec().index if ind in stop_words | commen_words]
        return self.vec().drop(stop_common_list) # Remove stop words and common words
...
class InfRet():
    ...
    def lsi(self,query, rank = 'opt'): # Rank can be chosen, otherwise by default rank is equal to "opt".
    ...
    ### Clustering
    def cluster(self, query, num_cluster):
    ...
    def nmf(self,query,num_components ): # num_components actually means number of topics
...
class AvgPrfm(InfRet):

    def ave_fullv(self, number_of_queries):
        all_qrys = []
        for q in range(number_of_queries):
            [rec,prec] =  InfRet.fullv(self,q+1)
            all_qrys.append([rec,prec])
        return self.for_display(number_of_queries,all_qrys)



    def ave_lsi(self,number_of_queries,rank):
        all_qrys = []
        for q in range(number_of_queries):
            rec,prec,apprx_error =  InfRet.lsi(self,q+1,rank)
            all_qrys.append([rec,prec])
        return self.for_display(number_of_queries,all_qrys)



    def ave_cluster(self,number_of_queries,num_cluster):
        all_qrys = []
        for q in range(number_of_queries):
            rec,prec,apprx_error =  InfRet.cluster(self,q+1,num_cluster)
            all_qrys.append([rec,prec])
        return self.for_display(number_of_queries,all_qrys)



    def ave_nmf(self,number_of_queries,num_components):
        all_qrys = []
        for q in range(number_of_queries):
            rec,prec,apprx_error =  InfRet.nmf(self,q+1,num_components)
            all_qrys.append([rec,prec])
    ...
    def ave_lgkb(self,number_of_queries,step):
        all_qrys = []
        for q in range(number_of_queries):
            rec,prec,apprx_error =  InfRet.lgkb(self,q+1,step)
            all_qrys.append([rec,prec])
        return self.for_display(number_of_queries,all_qrys)



    def for_display(self, number_of_queries,all_qrys):
        import numpy as np
        ...
        for i in range(number_of_queries):
            ...
            interpolate = interp1d(all_qrys[i][0],all_qrys[i][1])

            start_point = next(points[j] for j in range(len(points)) if min(all_qrys[i][0]) < points[j] )
            stop_point = next(points[len(points)-1-j] for j in range(len(points)) if max(all_qrys[i][0]) > points[len(points)-j-1])
